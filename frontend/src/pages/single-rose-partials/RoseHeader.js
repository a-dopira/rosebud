...
function RoseHeader() {
    ...
    return (
        <div className={`flex flex-col ${!isStackedLayout ? 'md:flex-row' : ''}`}>
            ...
            <div className={`${!isStackedLayout ? 'md:w-1/2' : 'w-full'} p-5 flex items-center justify-center`}>
                ...
                <div className="border-solid border-gray-300 border-[1px] rounded-lg p-5">
                    <div className="relative"> 
                        ...
                        <div className="flex items-center justify-center space-x-10">
                            <button className="inline-block btn-red text-xl h-11" onClick={() => setEditModal(true)}>Изменить</button>
                            <button className="inline-block btn-red text-xl h-11" onClick={() => setDeleteModal(true)}>Удалить</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            >
                <div className="text-sm">
                    <form encType="multipart/form-data" className="space-y-3" onSubmit={updateRose}>
                        <label className="block w-full mb-1" htmlFor="title">Изменить название</label>
                        <input className="form-input" type="text" name="title" defaultValue={rose.title}/>
                        
                        <label className="block w-full mb-1" htmlFor="title_eng">Изменить описание на англ</label>
                        <input className="form-input" type="text" name="title_eng" defaultValue={rose.title_eng}/>
                        
                        <label className="block w-full mb-1" htmlFor="const_width">Изменить ширину</label>
                        <input className="form-input" type="text" name="const_width" defaultValue={rose.const_width}/>
                        
                        <label className="block w-full mb-1" htmlFor="const_height">Изменить высоту</label>
                        <input className="form-input" type="text" name="const_height" defaultValue={rose.const_height}/>
                        
                        <label className="block w-full mb-1" htmlFor="breeder">Изменить селекционера</label>
                        <select 
                        ...
                        <label className="block w-full mb-1" htmlFor="landing_date">Изменить дату посадки</label>
                        <input className="form-input" type="date" name="landing_date" defaultValue={rose.landing_date}/>
                        
                        <label className="block w-full mb-1 font-bold">Выбрать файл:</label>
                        <input className="form-input" type="file" name="photo" />
                        
                        <label className="block w-full mb-1" htmlFor="description">Описание</label>
                        <textarea className="form-input" defaultValue={rose.description} name="description" rows="4"></textarea>
                        
                        <div className="flex space-x-2 mt-4">
                            <button className="flex-1 btn-red text-lg py-2" type="submit">Обновить</button>
                            <button className="flex-1 btn-gray text-lg py-2" type="button" onClick={() => setEditModal(false)}>Отмена</button>
                        </div>
                    </form>
                </div>
            </GenericModal>
